...
class MambaXrayCTLCLIP(pl.LightningModule):
    """
    MambaXrayCTL stage-2 image-text contrastive model.
    """
    def __init__(self, args):
        ...
        if args.type == 'base':
            ...
            print('Loading vision encoder with base model -- Done')
        else:
            self.visual_encoder = arm_large_pz16(args.type)
            print('Loading vision encoder with large model -- Done')
        ...
        if self.text_encoder_type == 'Bio_ClinicalBERT':  
            self.tokenizer = AutoTokenizer.from_pretrained(args.bio_clinicalbert_path)
            if self.tokenizer.bos_token_id is None:
                self.tokenizer.bos_token_id = self.tokenizer.cls_token_id
            self.text_encoder = AutoModel.from_pretrained(args.bio_clinicalbert_path)
        ...
        for name, param in self.named_parameters():
            if not param.is_contiguous():
                print(f'参数 {name} 不是连续的，正在将其转换为连续。')
