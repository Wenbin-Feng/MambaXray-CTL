...
class Mamba(nn.Module):
    def __init__(
        ...
        if bimamba_type == "v1":
            ...
            A_d_b_log = torch.log(A_d_b)  # Keep A_b_log in fp32
            self.A_d_b_log = nn.Parameter(A_d_b_log)
            self.A_d_b_log._no_weight_decay = True

            self.conv1d_d_b = nn.Conv1d(
                in_channels=self.d_inner,
                out_channels=self.d_inner,
                bias=conv_bias,
                kernel_size=d_conv,
            ...
            self.x_proj_d_b = nn.Linear(
                self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs
            )
            self.dt_proj_d_b = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs)

            self.D_d_b = nn.Parameter(torch.ones(self.d_inner, device=device))  # Keep in fp32
            self.D_d_b._no_weight_decay = True
        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)

    def forward(self, hidden_states, segmenttation_features = None, inference_params=None):
        """
        hidden_states: (B, L, D)
        ...
        if segmenttation_features is not None:
            xd = rearrange(
                self.in_proj.weight @ rearrange(segmenttation_features, "b l d -> d (b l)"),
                "d (b l) -> b d l",
                l=seqlen,
            )
            if self.in_proj.bias is not None:
                xd = xd + rearrange(self.in_proj.bias.to(dtype=xz.dtype), "d -> d 1")
