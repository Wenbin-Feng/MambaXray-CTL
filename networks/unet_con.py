...
class MLP(nn.Module):
    def __init__(self, input_channels=768, num_class=128):
        ...
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.f1 = nn.Linear(input_channels, 512)
        self.bn1 = nn.BatchNorm1d(197)
        self.f2 = nn.Linear(512, num_class)
        self.activation = nn.ReLU()
    def forward(self, x):
        #x = self.gap(x)
        #print(x.shape)
        y = self.f1(x)  #（60，197，512）
        y = self.bn1(y)
        y = self.activation(y)
        y = self.f2(y)  #（60，197，128）
        y = y.view(y.size(0), -1) #（60，197*128）
